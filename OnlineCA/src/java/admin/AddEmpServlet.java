...
public class AddEmpServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            db.pstmt = db.con.prepareStatement("insert into emp_mstr(ename,efname, dob, address,phone,email,doj, designation,login, pswd) values (?,?,?,?,?,?,?,?,?,?);");
            ...
            db.pstmt.setString(8, desg);
            db.pstmt.setString(9, elogin);
            db.pstmt.setString(10, pswd);
            int j = db.pstmt.executeUpdate();
            if (j > 0) {
                response.sendRedirect("adminAddEmp.jsp?msg=Added Successfully !");
            } else {
                response.sendRedirect("adminAddEmp.jsp?msg=Failed !");
            }
